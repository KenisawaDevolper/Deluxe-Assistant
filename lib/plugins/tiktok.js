...
async function fetchTikTokData(url) {
    ...
    try {
        const { data } = await axios.get(`https://api.tiklydown.eu.org/api/download?url=${encodeURIComponent(url)}`, { timeout: 15000 });
    ...
    async handler({ command, q, reply, sock, m }) {
        ...
        if (command === 'ttmp3') {
            ...
            try {
                ...
                const audioBuffer = (await axios.get(data.audioUrl, { responseType: 'arraybuffer', timeout: 60000 })).data;
        ...
        if (command === 'ttmp4') {
            ...
            try {
                const data = await fetchTikTokData(query);
                if (data.type === 'images') throw new Error('Este TikTok es una galería de fotos, usa .tiktok para ver el carrusel de imágenes.');
                const videoBuffer = (await axios.get(data.videoUrl, { responseType: 'arraybuffer', timeout: 90000 })).data;
                return await sock.sendMessage(m.chat, {
        ...
        if (data.type === 'images' && data.images.length > 0) {
            try {
                ...
                // Fallback si falla el carrusel
                await reply(`📸 *TikTok Galería (${data.images.length} imágenes)*\n👤 *Autor:* @${data.username}\n💬 *Título:* ${data.title}\n\nEnviando fotos...`);
                for (let i = 0; i < data.images.length; i++) {
                    const imgUrl = data.images[i];
                    try {
                        const imgBuffer = (await axios.get(imgUrl, { responseType: 'arraybuffer', timeout: 30000 })).data;
